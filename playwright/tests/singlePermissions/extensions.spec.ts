...
test("TC: WEENSPACE_10 User should be able to navigate to extensions list as a staff member using APP permission #e2e", async () => {
  await home.goto();
...
test("TC: WEENSPACE_131 User with MANAGE_APPS permission can install apps but not plugins on explore extensions page #e2e", async () => {
  await home.goto();
  await home.welcomeMessage.waitFor({ state: "visible", timeout: 30000 });
